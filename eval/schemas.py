...
class OpenAIParameter(BaseModel):
    ...
    properties: Dict[
        str, Dict[str, Any]
    ]  # Changed Dict[str, str] to Dict[str, Any] to handle nested types
...
class FunctionResults(BaseModel):
    ...
    def check_score(self, values_list: List[Any], functions_list: List[str]) -> float:
        ...
        # Count matching values and track missing ones
        missing_values = []
        matching_values = 0
        ...
        # Count matching functions and track missing ones
        missing_functions = []
        matching_functions = 0
        for function in functions_list:
            if function in self.function_results.keys():
